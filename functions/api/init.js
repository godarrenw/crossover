...
export async function onRequestPost(context) {
  try {
    ...
    if (!hasData) {
      // 插入初始数据
      const initialData = [
        ['2024-08', 10994.64, 3758.68, 0.00, 0.00, 4.0],
        ['2024-09', 8636.45, 8178.76, 0.00, 0.00, 4.0],
        ['2024-10', 14850.23, 8821.50, 0.00, 0.00, 4.0],
        ['2024-11', 26108.82, 6600.00, 304059.39, 9278.17, 4.0],
        ['2024-12', 9141.20, 6488.11, 293817.67, -11999.75, 4.0],
        ['2025-01', 24608.17, 3278.48, 311662.68, 7542.26, 4.0],
        ['2025-02', 8342.17, 14964.45, 303283.28, -690.37, 4.0],
        ['2025-03', 14374.44, 10565.32, 294391.20, -15569.97, 4.0],
        ['2025-04', 17059.36, 14836.63, 278823.86, -15760.14, 4.0],
        ['2025-05', 35879.91, 14181.04, 299630.47, 12250.89, 4.0],
        ['2025-06', 19575.01, 12053.54, 320615.49, 4159.86, 4.0],
        ['2025-07', 15757.19, 8983.79, 340392.94, 16693.96, 4.0],
        ['2025-08', 8507.45, 9101.34, 322839.15, -6354.37, 4.0]
      ];

      for (const [yearMonth, totalIncome, totalExpense, totalCapital, investmentIncome, interestRate] of initialData) {
        await DB.prepare(`
          INSERT INTO financial_data (year_month, total_income, total_expense, total_capital, investment_income, interest_rate) 
          VALUES (?, ?, ?, ?, ?, ?)
